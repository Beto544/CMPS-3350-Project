...
int main() {
	...
	int done = 0;
	initSound();
	playSound(gl.alSourceBackground);
	while (!done) {
...
int check_keys(XEvent *e) {
	...
	switch (key) {
		...
		case XK_2:
			if (shift) {
				if (gl.feature_mode > 0)
				    // Do something when the Q key is pressed
    				printf("The Q key was pressed!\n");
					RvFeaturemode();
					gl.feature_mode = 0;
			} else
				gl.feature_mode = 2;
		...
		case XK_3:
			if (gl.feature_mode ==2){}
				playSound(gl.alSourceWin);
			break;
		case XK_4:
			if (gl.feature_mode==3) {
				playSound(gl.alSourceDrip);
			}
			break;
		case XK_5:
			if (shift) {
...
void render() {
	...
	//Raul's feature mode
	if (gl.feature_mode == 2) {
		ggprint8b(&r, 16, 0x00ffffff, "Sound demo", 1);
		ggprint8b(&r, 16, 0x00ffffff, "3. win sound", 1);
		ggprint8b(&r, 16, 0x00ffffff, "4. cannon sound", 1);
	}
	...
	if (tankHit) {
		renderExplosion();
		renderFirework();//test for firework
	}
	if (boxHit) {
		renderExplosion();
	}
